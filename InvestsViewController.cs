...
namespace nucaapp
{
    public partial class InvestsViewController : UIViewController
    {
        UITableView table;
        bool useRefreshControl = false;
        UIRefreshControl RefreshControl;
        UISearchBar searchBar;
        List<InvestsModel> allnews;
        UIActivityIndicatorView spinner;

        public InvestsViewController(IntPtr handle) : base(handle)
        {
        }
        ...
        public override async void ViewDidLoad()
        {
            ...
            getInvests gn = new getInvests();
                List<InvestsModel> allnews = await gn.GetAllNews();
                if (allnews != null)
                {
                    table = new UITableView(new CGRect(0, 20, View.Bounds.Width - 5, View.Bounds.Height - 20));
                    table.AutoresizingMask = UIViewAutoresizing.All;
                    table.RowHeight = 120f;
                    table.EstimatedRowHeight = 120f;
                    table.Source = new ListsInvistsModelSource(allnews.ToArray(), this,"");
        ...
        private async Task searchTableAsync()
        {
            ...
            getInvests gn = new getInvests();
            List<InvestsModel> allnews = await gn.GetAllNews();
            table.Source = new ListsInvistsModelSource(allnews.ToArray(), this, searchBar.Text);
